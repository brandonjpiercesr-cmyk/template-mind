// ⬡B:tests.openrouter_seat_spend:TEST:exact_seat_daily_dollars_stop_before_egress:20260725⬡
'use strict';
...
test('the credential-resolved seat replaces an ambient channel guess at actual egress',
  {concurrency:false}, async function () {
    const saved = preserveEnv();
    const priorSpend = require.cache[SPEND_PATH];
    try {
      process.env.OR_KEY_CANON = 'canon-seat-key';
      process.env.SEAT_CANON_DAILY_CAP_USD = '2';
      ...
      await spend.withAttribution({seat:'c2_organ',component:'pai.cycle'}, async function () {
        const result = await guard.run(
          'https://openrouter.ai/api/v1/chat/completions',
          {headers:{Authorization:'Bearer canon-seat-key'}},
          async function () { return keyResponse(0.1); },
          async function () { assert.equal(spend.allow('text',{egress:true}),true);
            return new Response('paid'); });
        assert.equal(result.blocked,false);
      });
      assert.deepEqual(spend.usageAttribution().by_seat,[{owner:'canon',count:1}]);
      assert.deepEqual(spend.usageAttribution().by_component,[{owner:'pai',count:1}]);
    } finally {
      require(GUARD_PATH)._test.reset();
      restoreEnv(saved);
      delete require.cache[SPEND_PATH];
      if (priorSpend) require.cache[SPEND_PATH] = priorSpend;
    }
  });
