...
async function tick() {
  ...
  try {
    ...
    // ⬡B:cycle.runner:SUPERSEDE:template_zero_does_not_dispatch_canew_drain:20260725⬡
    // Build commissioning is not owned by a template world's hourly mind tick. The
    // former direct drain request was unauthenticated and could wake a coder with no
    // accepted work contract. CODA owns intentional build commissioning through its
    // governed queue and receipts. This runner has no compatibility request path.
    ...
    // but nothing called them -- shelf-ware. Now, after the cook and SPAN, the
