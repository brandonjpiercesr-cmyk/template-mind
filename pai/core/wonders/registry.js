...
const NODES = [
  ...
  },
  {
    ...
    owner_wonder_id:'wonder.anu', reports_to:'station.pai', ham_scope:'inherited',
    technical_role:'Reason over exact immutable same-HAM artifact evidence, publish linked supersede-only living knowledge views through the canonical PAI council, and approve the exact human-visible page bytes before read-only projection.',
    ...
    metadata:{provider_seat:'c1_cellm',spend_owner_node_id:'wonder.knowledge_compiler',
      wiring:[wire('core/knowledge.compiler.wonder.js#compile'),
        wire('core/knowledge.human.release.wonder.js#judge'),
        wire('core/knowledge.human.release.wonder.js#verify'),
...
const SEAT_LEGS = {
  ...
  'wonder.knowledge_compiler': {
    ...
    jd: { summary: 'Compile verified immutable artifacts into linked supersede-only knowledge views through the C1 penny seat and the full PAI council, then clear exact human page bytes for projection.',
      duties: ['Read every supplied immutable source and relevant prior view before deciding.', 'Cite claims only to verified raw evidence.', 'Use supersedes and related only for existing prior views.', 'Return update, no change, or wait through the exact compilation receipt.', 'Judge the complete title, body, and human claims for the person and write an exact release receipt before projection.'],
      never: ['Never edit or erase raw evidence.', 'Never invent a citation.', 'Never publish without council clearance and exact readback.', 'Never let a phrase list or cold score decide human meaning.', 'Never speak to a person directly.'] },
    goals: ['Living knowledge improves without losing source truth.', 'Every paid compilation belongs to this Wonder and its penny seat.', 'A replay never buys the same judgment twice.', 'Only exact pages carrying durable agreement reach a person.'],
    toolbelt: ['tool.brain.find', 'tool.model.ladder'],
