...
function verifiedTrivialVoiceGreeting(ctx, handoff) {
  ...
  // RESTORED after the 20260718 RE-GRAFT clobber. A bare greeting is fact-free on
  // EVERY channel; requiring a verified voice handoff silenced text greetings.
  handoff = handoff || verifiedVoiceCallHandoff(ctx) || {};
...
async function defaultShadowStage(ctx, injected) {
  ...
  // RESTORED after the RE-GRAFT clobber. A regex phrasing heuristic is EVIDENCE the
  // wonder weighs, never a veto; byte-level catches below remain unconditional.
  ...
  // RESTORED after the RE-GRAFT clobber. FOUNDER LAW: the verdict belongs to the
  // WONDER; deterministic proofs are evidence it weighs, never cold overrides.
  ...
  if (boardPassed && deterministicFindings.length === 0 && judgment && parsed &&
    ...
    // The wonder's final review verdict decides, in BOTH directions. Unavailable or
    // unparseable review on a clean board falls back to whether the prior hold
    // could quote one real sentence; an unquotable double-hold is noise and passes.
