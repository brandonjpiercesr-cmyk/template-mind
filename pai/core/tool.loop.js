  ...
  while (iter<MAX && !ans) {
    ...
    if (msg.tool_calls&&msg.tool_calls.length) {
      ...
      if (_budgetGroundNeeded) {
        msgs.push({ role:'system', content:
          'The get_budget_summary result above is this person\'s REAL, current budget. Answer their money question directly FROM it, in plain words, with its exact figures, and honor its incomePosture note if present. Their income is tracked as recurring SOURCES, so a logged totalIncome of 0 alongside projectedIncome entries is NORMAL and does NOT mean they have no income. When they ask about their budget monthly, give the true monthly run-rates the result carries: monthlyIncomeTotal, monthlyBillsTotal, and monthlyNet. Quote every dollar figure EXACTLY as the result gives it (a monthly run-rate, a per-source amount, or a window total); never round a figure to the nearest hundred or thousand. NEVER say their budget is not set up, or that they have no income or no bills, when the result shows projectedIncome, incomeSources, or recurringBills. Only if the result is genuinely empty (empty:true) do you say the budget is not set up yet.' });
      }
      continue;
    }
