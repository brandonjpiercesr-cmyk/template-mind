...
2. **PAI watchdog + heartbeat may not be running at all** — `core/pai/watchdog.js` writes an `ALERT` only when stale (:34), nothing when healthy (:71–74), and I found **no auto-start** (only the status route `routes/pai.routes.js:13`). Worse: `index.js:513` `heartbeat.startHeartbeat()` targets the **retired** `core/heartbeat.js`, which `require`s missing `./ABAHAM`/`./MemoryBank`/`./HealthMonitor` (:10–12) → throws at require → swallowed by the boot try/catch. **Net: the real PAI heartbeat/watchdog may not be beating unless someone POSTs `/pai/start`.** ← CODA should verify this wiring; if confirmed, it's a live gap (the heartbeat isn't).
